...
const MoolaConnector = artifacts.require("MoolaConnector");
...
module.exports = function (deployer, network, accounts) {
	return deployer.then(async () => {
		...
		store.setCentroContractAddress(centro.address);
		store.newAddressProvider(
			"moola",
			"0x6EAE47ccEFF3c3Ac94971704ccd25C7820121483"
		);
		const moola = await deployer.deploy(MoolaConnector);
		const exchange = await deployer.deploy(ExchangeConnector);
